...
begin
  ...
  repeat
   ...
   //       sled^.tmp1.ynow:=sled^.cd.y+sled^.sp.y;
		...
		if (sled^.tmp1.xleft=sled^.sp.x) then
		begin
		  sled^.tmp1.xleft:=sled^.sp.x-round((sled^.sp.x*sled^.tmp1.yleft)/sled^.sp.y);
		  sled^.tmp1.xnow:=sled^.cd.x+round((sled^.sp.x*sled^.tmp1.yleft)/sled^.sp.y)
		end;
		if (sled^.tmp1.yleft=sled^.sp.y) then
		begin
		  sled^.tmp1.yleft:=sled^.sp.y-round((sled^.sp.y*sled^.tmp1.xleft)/sled^.sp.x);
		  sled^.tmp1.ynow:=sled^.cd.y+round((sled^.sp.y*sled^.tmp1.xleft)/sled^.sp.x)
		end;
		...
		if (((sled^.tmp1.xleft+sled^.tmp1.xnow)>MaxX) or ((sled^.tmp1.xleft+sled^.tmp1.xnow)<0) or ((sled^.tmp1.yleft+sled^.tmp1.ynow)>MaxY) or ((sled^.tmp1.yleft+sled^.tmp1.ynow)<0)) then
        begin
